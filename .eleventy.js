...
export default function (eleventyConfig) {
  // src/docs/<slug>/ is git-ignored (regenerated by sync-docs) — but 11ty
  ...
  // Per-project doc collections, driven by src/_data/site.js. Each project's
  // pages are tagged `<slug>-docs` by scripts/fetch-docs.mjs, so a new project
  // gets its sidebar/pager for free once it is added to `projects[]`.
  const byOrder = (a, b) => (a.data.order ?? 0) - (b.data.order ?? 0);
  for (const project of site.projects ?? []) {
    const tag = `${project.slug}-docs`;

    eleventyConfig.addCollection(`${project.slug}Docs`, (api) =>
      api.getFilteredByTag(tag).sort(byOrder)
    );

    eleventyConfig.addCollection(`${project.slug}DocsBySection`, (api) => {
      const items = api.getFilteredByTag(tag).sort(byOrder);
